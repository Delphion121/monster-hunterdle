...
function stateKey(dayKey) {
    return `mh-state:${dayKey}`;

}

function loadState(dayKey) {
    const key = stateKey(dayKey)
    ...
    try {
        ...
        console.warn("Invalid save detected, resetting:", err)
...
function saveState(dayKey, state) {
    localStorage.setItem(stateKey(dayKey), JSON.stringify(state));
}
...
function pickDailyMonster(monsters, dayKey) {
    const seed = fnv1a32(dayKey);
    return monsters[seed % monsters.length];
}
...
async function getDailyMonster() {
    ...
    const dayKey = await getDayKeyPreferNetwork();
    dailyMonster = pickDailyMonster(monsters, dayKey);

    console.log("Day Key (ET):", dayKey);
    console.log("Daily Monster:", dailyMonster)

    setDailyMonsterDescription(dailyMonster.Description)

    window.__DAY_KEY__ = dayKey
}

async function getDayKeyPreferNetwork() {
    const networkNow = await getNetworkNow();
    return getEasternDayKey(networkNow ?? new Date());
}
...
async function init() {
   await getMonsterData();
   await getDailyMonster(); 

   const dayKey = window.__DAY_KEY__;
   if (!dayKey) return;

   const state = loadState(dayKey);

   for (const name of state.guesses) {
    const guessedMonster = getMonsterByName(name);
    if (!guessedMonster) continue;
...
function onSubmit(event) {
    ...
    // persist for todays random monster
    const dayKey = window.__DAY_KEY__;
    if (dayKey) {
        const state = loadState(dayKey);
        state.guesses.push(guessedMonster.Name)
        saveState(dayKey, state);
    } else {
        console.warn("Day key is not set yet, skipping save.");
    }
