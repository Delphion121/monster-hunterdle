...
// Fetch monster data for autocomplete and comparisons
async function getMonsterData() {
    const monsterRes = await fetch("monsters.json");
...
async function getDailyArmorMonster() {
    const response = await fetch("http://127.0.0.1:3000/currentArmorMonster");
    if (!response.ok) {
...
getMonsterData();
getDailyArmorMonster();


// Creating the Autocomplete function for text box
...
// When the button is pressed, run the appropriate functions - compareMonsters 
function onSubmit(event) {
    ...
    removeSuggestions(); // Clear suggestions
...
if (imageFilePath) {
    ...
    // Display the image to the player
} else {
